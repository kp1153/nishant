...
export default function StockForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-200 p-5 max-w-2xl">
      <div className="font-bold text-black mb-4 text-base">नया सामान जोड़ें</div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div className="sm:col-span-2">
          ...
          <input className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm outline-none focus:border-black"
            placeholder="जैसे: गेट वाल्व 1 इंच" value={form.naam}
            onChange={e => setForm({ ...form, naam: e.target.value })} />
        </div>
        <div>
          <label className="text-xs text-gray-500 mb-1 block">श्रेणी</label>
          <select className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm outline-none focus:border-black"
        ...
        <div>
          ...
          <input type="number" className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm outline-none focus:border-black"
          ...
          <div className="flex gap-2 flex-wrap">
            {gstList.map(g => (
              <button type="button" key={g} onClick={() => setForm({ ...form, gstDar: g })}
                className={`px-5 py-2 rounded-lg text-sm font-semibold border transition-all ${form.gstDar === g ? "bg-white text-black border-white" : "bg-white text-black border-white"}`}>
                {g}%
              </button>
            ))}
          </div>
      ...
      <button type="submit" disabled={loading}
        className="mt-5 bg-white text-black px-8 py-2.5 rounded-lg text-sm font-semibold">

      ...
      <div className="flex gap-2 flex-wrap">
        {shreniList.map(s => (
          <button key={s} onClick={() => setFilter(s === "सभी" ? "" : s)}
            className={`px-3 py-1.5 rounded-full text-xs font-semibold border transition-all ${(filter === s || (s === "सभी" && filter === "")) ? "bg-[#0f2d5e] text-white border-[#0f2d5e]" : "bg-white text-gray-600 border-gray-200 hover:border-[#0f2d5e]"}`}>
            {s}
          </button>
        ))}
      </div>

...
export default function StockPage() {
  ...
  const shreniList = ["???", ...masterCatalog.map(c => c.shreni)]
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold text-[#0f2d5e]">?? ?????</h1>
      ...
      <div className="flex gap-2 flex-wrap">
        {shreniList.map(s => (
          <button key={s} onClick={() => setFilter(s === "???" ? "" : s)}
            className={`px-3 py-1.5 rounded-full text-xs font-semibold border transition-all ${(filter === s || (s === "???" && filter === "")) ? "bg-blue-50 text-[#0f2d5e] border-[#0f2d5e]" : "bg-white text-gray-600 border-gray-200 hover:border-[#0f2d5e]"}`}>
            {s}
          </button>
        ))}
      </div>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center px-4">
          <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-lg">
            <div className="flex justify-between items-center mb-4">
              <div className="font-bold text-[#0f2d5e]">?? ????? ??????? ????</div>
      ...
      <div className="space-y-3 lg:hidden">
        {filtered.length === 0 ? (
          <div className="bg-white rounded-xl border border-gray-200 px-5 py-8 text-center text-gray-400 text-sm">??? ????? ????</div>
        ) : filtered.map((s) => (
          <div key={s.id} className="bg-white rounded-xl border border-gray-200 p-4">
            <div className="flex justify-between items-start">
              ...
              <div className="text-right">
                ...
                <div className="text-xs text-gray-400">{s.ikaai} ???</div>
              </div>
            </div>
            <div className="flex justify-between mt-2 pt-2 border-t border-gray-100 text-sm">
              <span className="text-gray-500">????: <span className="font-semibold text-gray-700">?{s.kharidMulya}</span></span>
              <span className="text-gray-500">??????: <span className="font-semibold text-[#0f2d5e]">?{s.bikriMulya}</span></span>
            </div>
      ...
      <div className="hidden lg:block bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={9} className="px-5 py-8 text-center text-gray-400">??? ????? ????</td></tr>
            ) : filtered.map((s) => (

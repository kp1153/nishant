    ...
    <>
      <aside className="hidden md:flex w-64 h-screen flex-col fixed top-0 left-0 bg-white border-r border-gray-200 shadow-sm">
        <div className="p-5 border-b border-gray-200">
          <div className="text-2xl font-extrabold text-blue-50 tracking-wide">निशांत</div>
          <div className="text-gray-400 text-xs mt-1">हार्डवेयर प्रबंधन</div>
          <div className="mt-4 flex items-center gap-3">
            <div className="w-9 h-9 rounded-full bg-gray-200 flex items-center justify-center text-gray-700 font-bold text-sm shadow">नि</div>
            <div>
              <div className="text-blue-50 text-xs font-semibold">व्यवस्थापक</div>
          ...
          {menu.map((item) => (
            <Link key={item.path} href={item.path}
              className={`flex items-center gap-3 px-5 py-2.5 text-sm font-medium transition-all rounded-lg mx-2 mb-0.5
                ${pathname === item.path ? "bg-blue-50 text-blue-50" : "text-gray-500 hover:bg-gray-50 hover:text-blue-50"}`}>
              <span>{item.icon}</span>
              <span>{item.naam}</span>
            </Link>
          ))}
        ...
        <button onClick={handleLogout}
          className="w-full flex items-center gap-3 px-5 py-3 text-sm font-medium text-gray-400 border-t border-gray-200 hover:bg-gray-50 hover:text-blue-50 transition-all flex-shrink-0">
          <span>🚪</span>
          <span>लॉगआउट</span>
        </button>
      </aside>
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 z-50 bg-white border-b border-gray-200 flex items-center justify-between px-4 py-3 shadow-sm">
        <div className="text-blue-50 font-extrabold text-lg tracking-wide">निशांत</div>
          ...
          item.path ? (
            <Link key={item.path} href={item.path}
              className={`flex-1 flex flex-col items-center justify-center py-2 text-[10px] font-medium transition-all
                ${pathname === item.path ? "text-blue-50" : "text-gray-400 hover:text-blue-50"}`}>
              <span className="text-xl mb-0.5">{item.icon}</span>
              {item.naam}
            </Link>
          ) : (
            <button key="meer" onClick={() => setDrawerOpen(true)}
              className="flex-1 flex flex-col items-center justify-center py-2 text-[10px] font-medium text-gray-400 hover:text-blue-50 transition-all">
              <span className="text-xl mb-0.5">{item.icon}</span>
              {item.naam}
            </button>
          )
        ...
        <div className="md:hidden fixed inset-0 z-50 flex flex-col justify-end">
          ...
          <div className="relative bg-white rounded-t-2xl p-4 pb-8">
            <div className="flex items-center justify-between mb-4">
              <div className="text-blue-50 font-semibold">सभी विकल्प</div>
              <button onClick={() => setDrawerOpen(false)} className="text-gray-400 text-xl">✕</button>
            </div>
            <div className="grid grid-cols-3 gap-3">
              {menu.map((item) => (
                <Link key={item.path} href={item.path} onClick={() => setDrawerOpen(false)}
                  className={`flex flex-col items-center gap-1 p-3 rounded-xl text-xs font-medium transition-all
                    ${pathname === item.path ? "bg-blue-50 text-blue-50" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>

          ...
          {menu.map((item) => (
            <Link key={item.path} href={item.path}
              className={`flex items-center gap-3 px-5 py-2.5 text-sm font-medium transition-all rounded-lg mx-2 mb-0.5
                ${pathname === item.path ? "bg-blue-50 text-[#0f2d5e]" : "text-gray-500 hover:bg-gray-50 hover:text-[#0f2d5e]"}`}>
              <span>{item.icon}</span>
              <span>{item.naam}</span>
            </Link>
          ))}
              ...
              {menu.map((item) => (
                <Link key={item.path} href={item.path} onClick={() => setDrawerOpen(false)}
                  className={`flex flex-col items-center gap-1 p-3 rounded-xl text-xs font-medium transition-all
                    ${pathname === item.path ? "bg-blue-50 text-[#0f2d5e]" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>

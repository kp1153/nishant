  ...
  return (
    <div className="flex flex-col lg:flex-row gap-4">
      <div className="flex-1 space-y-4">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          ...
          {selectedGrahak ? (
            <div className="flex items-center justify-between bg-blue-50 rounded-lg px-4 py-3">
              <div>
                <div className="font-semibold text-gray-800">{selectedGrahak.naam}</div>
                ...
                <div className="w-full bg-white border border-gray-200 rounded-lg mt-1 shadow-sm max-h-60 overflow-y-auto">
                  {filteredGrahak?.map((g) => (
                    <div key={g.id} onClick={() => { setSelectedGrahak(g); setSearchGrahak("") }}
                      ...
                      <div className="font-semibold text-sm text-gray-800">{g.naam}</div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          <div className="font-bold text-gray-700 mb-3">📦 सामान जोड़ें</div>
          <div>
            <input
              className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm outline-none focus:border-gray-400 text-gray-800"
                ...
                {filteredSamaan?.length === 0 ? (
                  ...
                  filteredSamaan?.map((s) => (
                    <div key={s.id} onClick={() => addItem(s)}
                      ...
                      <div>
                        <div className="font-semibold text-sm text-gray-800">{s.naam}</div>
                        <div className="text-xs text-gray-400">{s.shreni} · {s.matra} {s.ikaai} बचा · GST {s.gstDar ?? 18}%</div>
                      </div>
                      <div className="text-sm font-bold text-gray-700">₹{s.bikriMulya}</div>
                    </div>
                  ))
                )}
              </div>
                  ...
                  return (
                    <div key={i.id} className="border border-gray-100 rounded-lg p-3 bg-gray-50">
                      <div className="flex justify-between items-start">
                        <div className="font-semibold text-sm text-gray-800">{i.naam}</div>
                    ...
                    {items.map((i) => {
                      ...
                      return (
                        <tr key={i.id} className="border-t border-gray-50">
                          <td className="px-3 py-2 font-semibold text-gray-800">{i.naam}</td>
                          ...
                          <td className="px-3 py-2 text-right text-gray-700">₹{i.bikriMulya}</td>
                          ...
                          <td className="px-3 py-2 text-right font-bold text-gray-800">₹{kul}</td>
                          <td className="px-3 py-2 text-center">
      ...
      <div className="w-full lg:w-72 space-y-4">
        <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
          ...
          <div className="flex justify-between text-sm">
            ...
            <span className="font-semibold text-gray-800">₹{summary.base.toFixed(2)}</span>
          </div>
          <div className="flex justify-between text-sm">
          ...
          <div className="border-t border-gray-100 pt-2 flex justify-between">
            ...
            <span className="font-bold text-gray-900 text-lg">₹{summary.kul.toFixed(2)}</span>
          </div>
